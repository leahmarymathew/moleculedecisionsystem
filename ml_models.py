...
class MLModels:
    ...
    def __init__(self, features_df: pd.DataFrame):
        ...
        self.rf_scaler = None  # scaler fitted on RF training split
        self.xgb_model = None
        self.kmeans = None
    ...
    def save_models(self, models_dir) -> None:
        """Serialize trained models and scaler to disk."""
        models_dir = Path(models_dir)
        models_dir.mkdir(parents=True, exist_ok=True)

        if self.rf_classifier is not None:
            joblib.dump(self.rf_classifier, models_dir / 'rf_classifier.joblib')
        if self.rf_scaler is not None:
            joblib.dump(self.rf_scaler, models_dir / 'rf_scaler.joblib')
        if self.xgb_model is not None:
            joblib.dump(self.xgb_model, models_dir / 'xgb_model.joblib')
        if self.kmeans is not None:
            joblib.dump(self.kmeans, models_dir / 'kmeans.joblib')

        _, feature_names = self.prepare_feature_matrix()
        with open(models_dir / 'feature_names.json', 'w') as f:
            json.dump(feature_names, f)

    @classmethod
    def from_disk(cls, models_dir, features_df: pd.DataFrame) -> 'MLModels':
        """Load pre-trained models from disk for inference."""
        models_dir = Path(models_dir)
        instance = cls(features_df)

        rf_path = models_dir / 'rf_classifier.joblib'
        scaler_path = models_dir / 'rf_scaler.joblib'
        if rf_path.exists() and scaler_path.exists():
            instance.rf_classifier = joblib.load(rf_path)
            instance.rf_scaler = joblib.load(scaler_path)
        else:
            warnings.warn(f"No pre-trained RF model found in {models_dir}. Run train_models.py first.")

        xgb_path = models_dir / 'xgb_model.joblib'
        if xgb_path.exists():
            instance.xgb_model = joblib.load(xgb_path)
    ...
    def predict_all(self) -> None:
        """Run inference with all loaded models; falls back to neutral defaults if a model is missing."""
        X, _ = self.prepare_feature_matrix()

        if self.rf_classifier is not None and self.rf_scaler is not None:
            X_scaled = self.rf_scaler.transform(X)
            self.rf_predictions = self.rf_classifier.predict(X_scaled)
        else:
            self.rf_predictions = np.ones(len(self.features), dtype=int)  # All Medium (class 1)

        if self.xgb_model is not None:
            self.xgb_predictions = self.xgb_model.predict(X)
        else:
            self.xgb_predictions = np.zeros(len(self.features))

        if self.kmeans is not None:
            self.cluster_labels = self.kmeans.predict(X)
        else:
            self.cluster_labels = np.zeros(len(self.features), dtype=int)
