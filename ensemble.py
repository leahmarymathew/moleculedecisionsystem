...
class EnsembleDecisionEngine:
    ...
    def normalize_xgb_forecast(self) -> pd.Series:
        """
        FIX #19: Use percentile scaling instead of hardcoded formula.
        ...
        # FIX #19: Use percentile-based normalization instead of hardcoded division by 2
    ...
    def compute_confidence(self, 
        ...
        """
        FIX #20: Include data quality flags in confidence calculation.
        ...
        # FIX #20: Add data quality penalty for anomalies/flags
        data_quality_penalty = 1.0
        
        # Penalize if negative revenue flag
        if 'negative_revenue_flag' in self.scores.columns:
            data_quality_penalty = data_quality_penalty * 0.8  # 20% penalty
        
        # Penalize if imputed
        if 'imputed_flag' in self.scores.columns:
            imputed = self.scores['imputed_flag'].fillna(False)
            data_quality_penalty = data_quality_penalty * (1.0 - 0.15 * imputed.astype(float))  # 15% penalty if imputed
        
        # Penalize if anomaly detected
        if 'growth_anomaly_flag' in self.scores.columns:
            anomaly = self.scores['growth_anomaly_flag'].fillna(False)
            data_quality_penalty = data_quality_penalty * (1.0 - 0.25 * anomaly.astype(float))  # 25% penalty if anomaly
